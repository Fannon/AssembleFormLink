...
(function (mw, $) {
    ...
    mw.libs.AssembleFormLink.fetchValuesFromCategory = function() {
        ...
        $.each(categoriesToFetch, function(categoryName) {
            ...
            .done(function(json) {
                ...
                if (json && json.query && json.query.categorymembers) {
                    ...
                    affectedElements.each(function() {
                        $(this).append('<option value="" style="display:none;"></option>');
                        for (var i = 0; i < json.query.categorymembers.length; i++) {
                            var title = json.query.categorymembers[i].title;

                            // TODO: This should be part of a separate values-from-namespace option
                            if (title.indexOf(':') > -1) {
    ...
    mw.libs.AssembleFormLink.submit = function(event) {
        ...
        for (var i = 0; i < form.length; i++) {
            ...
            if (inputEl.type !== 'submit') {
                // If the input element has the option "stapled" and has content, put it in (braces)
                if ($(inputEl).attr('stapled') && form[i].value) {
                    url += ' (' + form[i].value + ')';
                } else {
                    url += form[i].value;
                }
        ...
        pageName = url.replace(form[0].value, '');
        ...
        // TODO: Uncompleted, won't work if Read API is not given
        //
        if($(form).attr('check-for-existence')) {
            ...
            .done(function(json) {
                ...
                if (json.query && json.query.pages && !json.query.pages['-1']) {
                    console.log('Checked in: ' + ((new Date().getTime()) - time) + 'ms :: Page already exists!');
                    alert('Die Seite existiert schon! Bitte ein Eindeutigkeitskriterium festlegen, falls sie als zweite, neue Seite angelegt werden soll.');
                    return false;
                }

                console.log('Checked in: ' + ((new Date().getTime()) - time) + 'ms :: Page does not exist yet, redirecting...');
                location.assign(url);
    ...
    $(function() {
        ...
        try {
            ...
            if (aflForm.length > 0) {
                ...
                // Enable Chameleon Tooltips, if available
                if ($.fn.tooltip) {
